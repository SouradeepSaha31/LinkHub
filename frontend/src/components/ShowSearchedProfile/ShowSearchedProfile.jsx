...
function ShowSearchedProfile(props) {
    const { user_id } = props
    const [user, setuser] = useState([])
    const [data, setdata] = useState([])
    const [scrollPosition, setScrollPosition] = useState(0);
    const [followers, setfollowers] = useState(200)
    const [followings, setfollowings] = useState(200)
    ...
    useEffect(() => {
      const fetchdata = async () => {
        try {
            ...
            const response = await axios.post(`/api/v1/searchuser/showsearchprofile/${user_id}`)
            setuser(response.data)
            
        } catch (error) {
          console.log("error in fetch data in showSearchProfile.jsx")
            console.log(error)
        }
      }
      fetchdata()
  ...
  useEffect(() => {
    ...
    const fetchdata = async () => {
      try {
        ...
        const response = await axios.post(`/api/v1/ownerpost/showpostinsearchprofile/${user_id}`)
        setdata(response.data)
        console.log(data)
        
      } catch (error) {
        console.log("error in fetch-data in ShowSearchedProfile.jsx")
        console.log(error)
      }
    }
  ...
  // follow click ---------------------------------------------------------------------------------
    const follow = async (userid) => {
      try {
        ...
        const response = await axios.post(`/api/v1/follow/followclick/${userid}`)
    ...
    useEffect(() => {
      const fetchdata = async () => {
        try {
          ...
          const response = await axios.post(`/api/v1/follow/followshow/${user_id}`)
...
const redirectToClickProfile = (user_id) => {
  if (user_id === loggedinUser._id) {
    navigate("/profile")
  } else {
    navigate(`/showsearchedprofile/${user_id}`)
    setlikeCompleted(!likeCompleted)
    settoggleFollowingsBox(false)
    settoggleFollowersBox(false)
  }
}
//------------------------------------------------------------------------------------------------
        ...
        <div className='min-h-screen w-4/5 px-10 py-5'>
          ...
          <div className='w-full h-[200px] flex'>
            <div className='w-1/5 h-full flex justify-center items-center'>
              ...
              <div className='w-[180px] h-[180px] rounded-full relative'>
                <img src= {user.avatar == "default-avatar.jpeg" ? `../public/default/${user.avatar}` : `${user.avatar}`} className='w-full h-full rounded-full object-cover object-center absolute top-0 left-0 z-2' alt="" />
            ...
            <div className='w-4/5 h-full flex '>
              <div className='w-1/2 h-full flex flex-col px-5 py-1 items-start gap-1 justify-start'>
                <div className='flex items-center justify-start gap-4'>
                  ...
                  <div onClick={() => follow(user._id)} className={`${following.indexOf(user_id) === -1 ? "bg-blue-700" : "bg-zinc-800"} cursor-pointer rounded-[5px] px-3 py-1 text-white font-semibold text-[17px]`}>{following.indexOf(user_id) === -1 ? "Follow" : "Following"}</div>
              ...
              <div className='w-1/2 h-full flex flex-col items-end justify-between gap-4'>
                ...
                <div onClick={() => followersBox(user_id)} className='min-w-[50px] h-[50px] cursor-pointer flex flex-col justify-center items-center'>
                  <h1 className='text-white text-[20px] font-semibold'>{followers}</h1>
                  <h1 className='text-white text-[17px]'>Followers</h1>
                </div>
                <div onClick={() => followingsBox(user_id)} className='min-w-[50px] h-[50px] cursor-pointer flex flex-col justify-center items-center'>
          ...
          <div className='w-full min-h-[180px] mt-[40px] relative'>
            ...
            <div className='w-full mb-5 h-[40px] flex justify-center border-t-[1px] border-zinc-700 items-center gap-2'>
              <div className= "border-zinc-100 text-white h-full w-[100px] text-[15px] font-semibold border-b flex justify-center items-center">Post</div>
            </div>
              ...
              <div className="your-box w-full h-[90%] overflow-y-auto py-1 relative">

                {
                  ...
                  userWhoAreLike.slice().reverse().map((whoLikes, index) => (
                    ...
                    {
                      whoLikes.userId === loggedinUser._id ? (
                        <></>
                      ) : (
              ...
              <div className="your-box w-full h-[90%] overflow-y-auto py-1 relative">

                {
                  ...
                  userWhoAreFollowers.slice().reverse().map((followers, index) => (
                    ...
                    {
                      followers.followersDet._id === loggedinUser._id ? (
                        <></>
                      ) : (

                    <div onClick={() => follow(followers.followersDet._id)} className={`${following.indexOf(followers.followersDet._id) === -1 ? "bg-blue-500 text-white" : "bg-zinc-800 text-zinc-500"} cursor-pointer text-[17px] font-semibold px-3 py-1 rounded-[10px]`}>
                        {following.indexOf(followers.followersDet._id) === -1 ? "Follow" : "Following"}
                    </div>
                      )
                    }
                </div>
              ...
              <div className="your-box w-full h-[90%] overflow-y-auto py-1 relative">

                {
                  ...
                  userWhoAreFollowings.slice().reverse().map((followings, index) => (
                    ...
                    {
                      followings.followingsDet._id === loggedinUser._id ? (
                        <></>
                      ) : (

                    <div onClick={() => follow(followings.followingsDet._id)} className={`${following.indexOf(followings.followingsDet._id) === -1 ? "bg-blue-500 text-white" : "bg-zinc-800 text-zinc-500"} cursor-pointer text-[17px] font-semibold px-3 py-1 rounded-[10px]`}>
                        {following.indexOf(followings.followingsDet._id) === -1 ? "Follow" : "Following"}
                    </div>
                      )
                    }
                </div>
...
export default ShowSearchedProfile
