...
function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchdata = async () => {
      try {
        ...
        const response = await axios.get("/api/v1/ownerpost/showpostinprofile")
  ...
  const logout = async () => {
    try {
      ...
      localStorage.setItem("saveposts", response.data.token)
      localStorage.setItem("likeposts", response.data.token)
      localStorage.setItem("following", response.data.token)
      localStorage.setItem("likecomments", response.data.token)
      
      toast.success("Logout in few seconds")
      setTimeout(() => {
        navigate("/login")
      }, 2500)
    } catch (error) {
      console.log("Unsuccessfully not logout")
  ...
  const changeAvatar = async (e) => {
    ...
    if(path.extname(avatar.name) === ".jpg" || path.extname(avatar.name) === ".jpeg" || path.extname(avatar.name) === ".png") {
      try {
        ...
        toast.success("Change Avatar in few seconds")
        setTimeout(() => {
          navigate("/profile")
        }, 2500)
      } catch (error) {
  ...
  const changeCover = async (e) => {
    ...
    if(path.extname(cover.name) === ".jpg" || path.extname(cover.name) === ".jpeg" || path.extname(cover.name) === ".png") {
      try {
        const formData = new FormData()
        formData.append("cover", cover)
        const response = await axios.post("/api/v1/user/uploadcoverimage", formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
        localStorage.setItem("user", JSON.stringify(response.data.user))
        toast.success("Change Cover Image in few seconds")
        setTimeout(() => {
          navigate("/profile")
        }, 2500)
      } catch (error) {
        console.log("error in change Cover: ", error)
        alert(`${error.response.status} , ${error.response.statusText} \n ${error.response.data.message}`)
      }
    } else {
      alert("Please select only image files")
...
/>
        ...
        <div className='min-h-screen w-4/5 px-10 py-5'>
          ...
          <div className='w-full mb-5 h-[200px] rounded-[20px] relative'>

            <img src= {user.coverImage == "cover-default.jpg" ? `./public/default/${user.coverImage}` : `${user.coverImage}`} className='w-full h-full rounded-[20px] object-cover object-center absolute top-0 left-0 z-2' alt="" />

            <label htmlFor='coverimage' className='w-[40px] h-[40px] bg-white absolute bottom-0 right-0 z-5 -translate-x-1 -translate-y-1 rounded-[17px] cursor-pointer flex justify-center items-center'>
                <IoCamera className='text-balck text-[20px]'/>
            </label>

            <input type="file" id='coverimage' name='coverimage' formEncType='multipart/form-data' onChange={changeCover} className='hidden'/>
          ...
          <div className='w-full h-[200px] flex'>
            <div className='w-1/5 h-full flex justify-center items-center'>
              ...
              <div className='w-[180px] h-[180px] rounded-full relative bg-gradient-to-r from-yellow-400 to-red-700 flex justify-center items-center'>
                <img src= {user.avatar == "default-avatar.jpeg" ? `./public/default/${user.avatar}` : `${user.avatar}`} className='w-[175px] h-[175px] border-4 border-black rounded-full object-cover object-center z-2' alt="" />

                <label htmlFor='avatar' className='w-[40px] h-[40px] bg-white absolute bottom-0 right-0 z-5 -translate-x-1 -translate-y-1 rounded-full cursor-pointer flex justify-center items-center'>
                <IoCamera className='text-balck text-[20px]'/>
                </label>

                <input type="file" id='avatar' name='avatar' formEncType='multipart/form-data' onChange={changeAvatar} className='hidden'/>
              </div>
            ...
            <div className='w-4/5 h-full flex '>
              <div className='w-1/2 h-full flex flex-col px-5 py-1 items-start gap-1 justify-start'>
                <div className='flex items-center justify-start gap-4'>
                  ...
                  <Link to="/editprofile" className='bg-yellow-500 rounded-[5px] px-3 py-1 text-white font-semibold text-[17px]'>Edit Profile</Link>
                </div>
              ...
              <div className='w-1/2 h-full flex flex-col items-end justify-between gap-4'>

                <div className='flex justify-center items-center gap-4'>

                <div className='min-w-[50px] h-[50px] flex flex-col justify-center items-center'>
                  <h1 className='text-white text-[20px] font-semibold'>{data.length}</h1>
                ...
                <button onClick={logout} className='px-5 py-2 cursor-pointer rounded-[10px] text-white text-[17px] font-semibold bg-red-600'>LogOut</button>
          ...
          <div className='w-full min-h-[180px] mt-[40px] relative'>
            ...
            <div className={`w-full min-h-[250px] flex ${data.length === 0 ? "justify-center" : "justify-start"} items-center flex-wrap gap-1`}>
              {
                data.length === 0 ? (
                  <h1 className='text-green-500 text-[20px] font-semibold'>no post yet</h1>
              ...
              <div className="your-box w-full h-[90%] overflow-y-auto py-1 relative">

                {
                  ...
                  userWhoAreFollowers.slice().reverse().map((followers, index) => (
                    ...
                    <div onClick={() => removeFollower(followers.followersDet._id)} className="bg-zinc-800 text-zinc-500 cursor-pointer text-[17px] font-semibold px-3 py-1 rounded-[10px]">
                        Remove
                    </div>
                </div>
